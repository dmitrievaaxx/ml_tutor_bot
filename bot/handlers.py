...
async def handle_complete_topic(message: Message):
    """
    Обработка команды /complete_topic для тестирования
    
    Позволяет отметить тему как завершенную для тестирования системы прогресса
    ...
    logger.info(f"Команда /complete_topic от пользователя {user_id} (@{username})")
    
    # Для тестирования отметим несколько тем как завершенные
    from bot.progress import mark_topic_completed, MATH_COURSE_TOPICS
    
    # Отмечаем первые 3 темы как завершенные
    for topic in MATH_COURSE_TOPICS[:3]:
        mark_topic_completed(user_id, topic)
    
    await message.answer(
        "✅ **Тестирование системы прогресса**\n\n"
        "Отмечены как завершенные первые 3 темы курса Math:\n"
        "• Векторы и операции\n"
        "• Матрицы и основные операции\n"
        "• Собственные значения и векторы\n\n"
        "Теперь используйте /learn → Math → Посмотреть план курса, "
...
async def handle_course_action(callback_query: CallbackQuery):
    ...
    if callback_query.data == "show_math_plan":
        # Показываем план курса Math с прогрессом
        from bot.progress import format_course_progress_text
        user_id = callback_query.from_user.id
        math_plan = format_course_progress_text(user_id, 'math')
        ...
        # Показываем план курса ML с прогрессом
        from bot.progress import format_course_progress_text
        user_id = callback_query.from_user.id
        ml_plan = format_course_progress_text(user_id, 'ml')
...
def register_handlers(dp: Dispatcher):
    ...
    # Обработчик команды /complete_topic - тестирование прогресса
    dp.message.register(handle_complete_topic, Command("complete_topic"))
