...
• /upload — загрузить PDF для анализа
...
**📄 Новая функция - Анализ PDF статей:**
...
• Примеры вопросов: "О чем статья?", "Какие эксперименты проводились?"
...
Используйте `/start` для возврата в главное меню."""
    ...
    await message.answer(exit_text, parse_mode="Markdown")
...
async def handle_pdf_file(message: Message):
    ...
    try:
        ...
        if result['success']:
            ...
            success_text += "• В чём основная идея и вклад авторов?\n"
            success_text += "• Какую задачу решает статья?\n"
            success_text += "• Какие методы или модели использованы?\n"
            success_text += "• Какие результаты и выводы сделали авторы?\n"
            success_text += "• Как этот подход сравнивается с другими?\n"
            success_text += "• Какие ограничения или слабые стороны отмечены?\n\n"
            
            # Добавляем предложенные темы
            if topics:
...
async def get_rag_response(query: str, user_id: int, dialog_history: list) -> str:
    """Получение ответа через улучшенную RAG систему с анализом качества"""
    try:
...
1. СНАЧАЛА проанализируй, есть ли ответ на вопрос в контексте документа выше
2. ЕСЛИ ответ ЕСТЬ в документе:
   - Используй ТОЛЬКО информацию из контекста документа
   - НЕ добавляй информацию из своих знаний
   - Начни ответ с "📄 Ответ на основе документа:"
3. ЕСЛИ ответа НЕТ в документе:
   - Начни ответ с "❌ В документе нет информации об этом."
   - Затем дай ответ на основе своих знаний
   - Укажи, что это общая информация, не из документа

ПРИМЕРЫ:
- Если в документе есть описание метода X, отвечай только на основе документа
- Если спрашивают про метод Y, которого нет в документе, скажи "В документе нет информации" и дай общий ответ
...
Отвечай на русском языке."""
        ...
        # Добавляем информацию об источнике только если LLM не добавил префикс
        if response and not response.startswith("📄 Ответ на основе документа:") and not response.startswith("❌ В документе нет информации об этом."):
            response = f"📄 Ответ на основе документа:\n\n{response}"
        
        # Добавляем перенос строки после префиксов, если его нет
        if response.startswith("📄 Ответ на основе документа:"):
            response = response.replace("📄 Ответ на основе документа:", "📄 Ответ на основе документа:\n\n")
        elif response.startswith("❌ В документе нет информации об этом."):
            response = response.replace("❌ В документе нет информации об этом.", "❌ В документе нет информации об этом.\n\n")
